...
<aside class="right-side">                
  ...
  <section class="content-header">
   ...
   <span class="total-left">Current Page: {{$productsCount}}</span>
   <ol class="breadcrumb">
          ...
          <table class="table table-advance" >
           ...
           <!--   <th ><input type="text" placeholder="Enter Name" name="first_name" value=""></th> 
              ...
              <th><span style="float: right;">
                <a href="{{url('admin/add-product')}}" class="btn btn-default">Add New Product</a>
            ...
            <tbody id="rowbody">
             ...
             <tr>
              <th scope="col">
               ...
               <td>{{$country->name}}</td>
               <td>
			   @if(!empty($country->product_category))
			   {{$country->product_category->menu_name}}
			   @endif			   
			   </td>
			   <td>{{$country->addedby->name}}</td>
               <td><img style="width: 50px;height: 40px;" src="{{url($country->img)}}"></img></td>
...
@if(Auth::user()->roles_id == 3)
              <td>
                <a title="Edit" href="{!! url('admin/product/edit-product',[$country->id]) !!}">
                  <i class="fa fa-pencil"></i>
                </a>
              </td> 
@else
            <td>
                <a title="View" href="{!! url('admin/product/edit-product',[$country->id]) !!}">
        ...
        for(i=0;i<element_lenght;i++)
        {
         if(country.elements[i].name=="mul_del[]")
         {
          if(country.elements[i].checked==true)
          {
           ...
           {
             $("#country").attr("action", "{{url('admin/deleteallproduct')}}");
             this.country.submit();
             break;
           }
         }	
