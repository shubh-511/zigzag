...
<aside class="right-side">                
                ...
                <section class="content-header">
                    <h1>Service Providers ({{$usersCount}})  </h1>
                    <span class="total-left">Current Page: {{count($users)}}</span>
                    <ol class="breadcrumb">
...
<form id="user" name="user" action="" method="post">
    ...
    <?php /*< <a title="Delete" href="#" onClick="deleteMe({{ $user->id}})"><i class="fa fa-trash-o"></i></a>
            */?>
            ...
            <td>
                <a href="{{ url('admin/service-provider/view-profile',[$user->id])}}"><i title="View Profile" class="fa fa-desktop"></i></a>
            </td>
...
function isValid(formRef)
 ...
 {     
    ...
    url: "../admin/updateprovidersstatus",
        ...
        for(i=0;i<element_lenght;i++)
        {
            if(user.elements[i].name=="mul_del[]")
            {
                if(user.elements[i].checked==true)
                {
                    ...
                    {
                        $("#user").attr("action", "{{url('admin/deleteallprovider')}}");
                        this.user.submit();
                        break;
                    }
                }   
