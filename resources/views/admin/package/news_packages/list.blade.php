          ...
          <table class="table table-advance" >
           ...
           <!--   <th ><input type="text" placeholder="Enter Name" name="first_name" value=""></th> 
              ...
              <th><span style="float: right;">
                <a href="{{url('admin/add-news-package')}}" class="btn btn-default">Add New</a>
       ...
       <div class="box-body table-responsive">
        <table id="example1" class="table table-bordered table-striped">
          <thead>
            ...
            <tbody id="rowbody">
             @foreach($packages as $country )
             <tr>
              <th scope="col">
               <input type="checkbox" class="check_box" name="mul_del[]" id="mul_del[]" value="{{ $country->id }}" /></th>
               
               <td>{{$country->price}}</td>
               <td>@if($country->access == 1) {{'Unlimited'}} @else {{$country->access}} Articles @endif</td>
               <td>For {{$country->period}} Month</td>
              ...
              <td>
                <a title="Edit" href="{!! url('admin/news-packages/edit-news-package',[$country->id]) !!}">
    ...
    for(var i=0;i<formRef.elements.length;i++)
    {
      ...
      {     
        $.ajax({
          ...
          url: "../admin/updatenewspackagestatus",
        ...
        for(i=0;i<element_lenght;i++)
        {
         if(country.elements[i].name=="mul_del[]")
         {
          if(country.elements[i].checked==true)
          {
           ...
           {
             $("#country").attr("action", "{{url('admin/deleteallnewspackages')}}");
             this.country.submit();
             break;
           }
         }	
