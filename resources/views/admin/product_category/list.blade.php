          ...
          <table class="table table-advance" >
           ...
           <!--   <th ><input type="text" placeholder="Enter Name" name="first_name" value=""></th> 
              ...
              <th><span style="float: right;">
                {{--<a href="{{url('admin/add-product-category')}}" class="btn btn-default">Add New Category</a>
       ...
       <div class="box-body table-responsive">
        <table id="example1" class="table table-bordered table-striped">
          <thead>
            ...
            <tbody id="rowbody">
             @foreach($product_cats as $country )
             <tr>
              <th scope="col">
               <input type="checkbox" class="check_box" name="mul_del[]" id="mul_del[]" value="{{ $country->id }}" /></th>
               <td>{{$country->name}}</td>
               <td><img style="width:50px;" src="{{asset($country->cat_img)}}"></img></td>
              ...
              <td>
                <a title="Edit" href="{!! url('admin/product-category/edit-product-category',[$country->id]) !!}">
    ...
    for(var i=0;i<formRef.elements.length;i++)
    {
      ...
      {     
        $.ajax({
          ...
          url: "../admin/updateproductcatstatus",
        ...
        for(i=0;i<element_lenght;i++)
        {
         if(country.elements[i].name=="mul_del[]")
         {
          if(country.elements[i].checked==true)
          {
           ...
           {
             $("#country").attr("action", "{{url('admin/deleteallproductcat')}}");
             this.country.submit();
             break;
           }
         }	
